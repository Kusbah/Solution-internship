...
const Todo = ({todo,toggleTodo,deleteTodo,editTodo,mode}) => {
  return (
    <div className={`todos-todo ${todo.done ? 'done' : ''}`}>
        ...
        <div className='todos-todo_cta-edit' onClick={() => editTodo (todo)}><FeatherIcon icon='edit'/></div>
        <div className='todos-todo_cta-delete' onClick={()=>deleteTodo(todo.id)}><FeatherIcon icon='trash-2'/></div>
        </div>
      )}
    </div>
  )
